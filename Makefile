...
LINK ?= dynamic
CARGO_ARGS := --release
CARGO_ARGS += $(if $(filter dynamic,$(LINK)),--no-default-features --features fltk/fltk-config --features fltk/system-libjpeg --features fltk/system-libpng --features fltk/system-zlib,--features bundled)
